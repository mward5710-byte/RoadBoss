...
async def seed(force: bool = False):
    # Force=true wipes demo collections and reseeds (used to refresh demo data without restart)
...
async def _wipe_demo_collections():
    """Wipe all demo data so a force reseed starts from a clean slate."""
    for coll in [
...
def _dated(payload: Dict[str, Any], when: datetime) -> Dict[str, Any]:
    """Like _make_doc but lets us backdate created_at/updated_at for realistic history."""
    payload['id'] = str(uuid.uuid4())
...
async def _seed_demo():
    ...
    # Founder + fleet admin + 1 dispatcher + 5 drivers (one per truck, all loggable)
    user_specs = [
    ...
    # Each historical trip: backdated created_at, started_at, ended_at, miles, mileage entries
    # Format: (driver_idx, vehicle_idx, origin, dest, miles, status, days_ago, hours_duration, state_breakdown)
    trip_specs = [
        # ACTIVE NOW
        (0, 0, 'Dallas, TX', 'Memphis, TN', 452.7, 'active', 0, None, None, 8),  # 8 hours into it
    ...
    for spec in trip_specs:
        ...
        when = NOW - timedelta(days=days_ago, hours=4)  # created in the morning
        trip_doc = _dated({
        ...
        # Mileage entries for IFTA (only for completed trips with breakdown)
        if mileage_split:
    ...
    # A typical day: 06:00 on_duty -> 07:00 driving -> 12:00 off_duty (lunch) -> 13:00 driving -> 19:00 off_duty -> sleeper overnight
    duty_cycle = [
    ...
    for d_idx, d in enumerate(drivers):
        for days_ago in range(7, 0, -1):
            ...
            for hour, status_v, note in duty_cycle:
                when = base + timedelta(hours=hour, minutes=(d_idx * 7) % 30)  # stagger drivers
        ...
        # Today's current status as final entry
        await db.hos_logs.insert_one(dict(_dated({
            'driver_id': d['id'],
    ...
    for m in maint_specs:
        when = NOW + timedelta(days=m['due_in_days']) if not m['completed'] else NOW + timedelta(days=m['due_in_days'])
        doc = _dated({
    ...
    for a in alert_specs:
        ...
        doc = _dated({
            ...
            'acknowledged': a['days_ago'] > 1,  # older alerts auto-acknowledged
        }, when)
        await db.alerts.insert_one(dict(doc))

    # ---------------- Dashcam events (15 spread over last 7 days) ----------------
    # Tuple format: (days_ago, hours_ago, vehicle_idx, driver_idx, event, severity, vendor, thumbnail_url_or_None)
    cam_specs = [
    ...
    dvir_plan = [
        # (driver_idx, days_ago, type, defect_key_or_None, defect_note_or_None)
    ...
    crash_specs = [
        # 1. False-positive dismissed (driver tapped "I'm OK" in the 15s window)
        ...
        # 2. Real low-impact crash, acknowledged + resolved
        ...
        # 3. Recent confirmed-unack (still hot for the demo) — high severity
    ...
    for c in crash_specs:
        ...
        await db.crash_events.insert_one(dict(doc))
        # If confirmed unack, also drop a critical alert for the demo home screen
    ...
    roadside_specs = [
        # Completed: tire blowout, full lifecycle
        ...
        # Cancelled: driver self-resolved
